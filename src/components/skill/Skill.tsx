    ...
    <div>
      <div className={`flex ${widthClass} ${heightClass} gap-2 mb-2`}>
        {icons.map((Icon, index) => (
          <Icon key={index} className="grayscale hover:filter-none transition duration-300" />
        ))}
      </div>
      <div className="w-full bg-gray-200 rounded-full h-1 mb-4 dark:bg-gray-700">
        <div className={`water-stream h-1 rounded-full dark:bg-blue-500 ${progressWidth}`}></div>
      </div>
    </div>
  );
};
  ...
  return (
    <section id="skill" className="w-full max-w-screen-md flex flex-col mt-8 mb-16">
      ...
      <div className="flex flex-col gap-4">
        ...
        />
        <SkillGroup 
          icons={[NodejsIcon, ExpressjsIcon, NestJsIcon, ReactJsIcon, NextjsIcon]} 
